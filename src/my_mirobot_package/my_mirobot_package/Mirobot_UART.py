...
class Mirobot_UART(WLKATA_UART):
    ...
    # 夹爪控制函数
    #{1}Status：
    # 0：Turn off the air pump;
    # 40: Open the gripper;
    # 60: Close the gripper;
    ...
    #{1}Status：
    # 0：Turn off the air pump;
    # 500: Air pump blowing;  吹气
    # 1000: Air pump suction;  吸气
    ...
    def __error_except(self, f, num):
        if num == 1:
            print(f"{f.__name__}: No reply - 'ok'")
        elif num == 2:
    ...
    def getStatus(self):
        ...
        if self.pSerial.in_waiting > 0:
            self.line = self.pSerial.readline().decode('utf-8').strip()
            #print(f"Raw response: {self.line}")  # Debugging statement
            if self.line[0] == "<" and self.line[-1] == ">":
                self.data = self.__parse_response(self.line)
                if isinstance(self.data, dict):
                    return self.data
                else:
                    print("Parsing error, returning default state")  # Debugging statement
                    return -1  # Return default state
            else:
                print("Invalid response format")  # Debugging statement
                return -1
        else:
            print("No response received")  # Debugging statement
            return -1
